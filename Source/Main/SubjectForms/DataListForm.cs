...
namespace Main.SubjectForms
{
    public partial class DataListForm : Form
    {
        ...
        {
            string sql = "select * from Subject where flag="+Flag;
            if (!string.IsNullOrEmpty(tbKeywords.Text.Trim()))
            {
                sql += (" and " + string.Format("subjectname like '%{0}%'", tbKeywords.Text));
            }
        ...
        {
            string sql = "delete from Subject where flag="+Flag+" id=@id";
